...
var zoomable = require('d3-zoomable');
...
var zoomable__default = /*#__PURE__*/_interopDefaultLegacy(zoomable);
...
var icicle = Kapsule__default['default']({
  props: {
    width: {
  ...
  stateInit: function stateInit() {
    return {
      zoom: zoomable__default['default']()
    };
  },
  ...
  update: function update(state) {
    ...
    var nodeClassNameOf = accessorFn__default['default'](state.nodeClassName);
    var animate = !state.skipTransitionsOnce;
    state.skipTransitionsOnce = false;
    var transition = d3Transition.transition().duration(animate ? state.transitionDuration : 0);
    var x0 = {
      td: function td(d) {
        return d.x0;
      },
    ...
    if (state.showLabels) {
      ...
      allCells.select('text.path-label').classed('light', function (d) {
        return !tinycolor__default['default'](colorOf(d.data, d.parent)).isLight();
