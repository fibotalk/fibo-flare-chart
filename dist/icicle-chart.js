// Version 1.5.0 icicle-chart - https://github.com/vasturiano/icicle-chart
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
  typeof define === 'function' && define.amd ? define(factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Icicle = factory());
  ...
  function zoom() {
    ...
    function mousedowned(event, ...args) {
      ...
      var currentTarget = event.currentTarget,
          ...
          p = d3Pointer(event, currentTarget),
          x0 = event.clientX,
          y0 = event.clientY;
      ...
      function mousemoved(event) {
        ...
        g.event(event)
         .zoom("mouse", constrain(translate(g.that.__zoom, g.mouse[0] = d3Pointer(event, currentTarget), g.mouse[1]), g.extent, translateExtent));
      }
    ...
    function dblclicked(event, ...args) {
      if (!filter.apply(this, arguments)) return;
      var t0 = this.__zoom,
          p0 = d3Pointer(event.changedTouches ? event.changedTouches[0] : event, this),
          p1 = t0.invert(p0),
    ...
    function touchstarted(event, ...args) {
      ...
      nopropagation(event);
      for (i = 0; i < n; ++i) {
        t = touches[i], p = d3Pointer(t, this);
  ...
  var zoomable = index$1({
    ...
    methods: {
      ...
      zoomReset: function zoomReset(state) {
        ...
        if (state.initialised) {
          state.transitionDuration = duration;
          state.el.call(state.zoom.transform, identity);
        }
      ...
      zoomTo: function zoomTo(state, _ref) {
        ...
        if (state.initialised) {
          state.transitionDuration = duration;
          state.el.call(state.zoom.transform, identity.scale(k).translate(x, y));
        }
  ...
  var LABELS_WIDTH_OPACITY_SCALE = linear().domain([4, 8]).clamp(true); // px per char

  var LABELS_HEIGHT_OPACITY_SCALE = linear().domain([15, 40]).clamp(true); // available height in px
  ...
  var icicle = index$1({
    ...
    methods: {
      ...
      _parseData: function _parseData(state) {
        if (state.data) {
          var hierData = hierarchy(state.data, index(state.children)).sum(index(state.size));
          ...
          if (state.excludeRoot) {
            ...
            var yScale = linear().domain([hierData.y1 - hierData.y0, size[1]]).range([0, size[1]]);
    ...
    init: function init(domNode, state) {
      ...
      var el = d3Select(domNode).append('div').attr('class', 'icicle-viz');
    ...
    update: function update(state) {
      ...
      var nameOf = index(state.label);
      var colorOf = index(state.color);
      var nodeClassNameOf = index(state.nodeClassName);
...
//# sourceMappingURL=icicle-chart.js.map
